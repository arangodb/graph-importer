...
- _database options._ These options describe the connection to the database and the objects that will be created in the
  database.
    ...
    - `--vertex_collection_name`: the name of the new vertex collection that will be created in the database, 
  default is `vertices` 
    - `--edge_collection_name`: the name of the new edge collection that will be created in the database,
  default is `edges`
    - `--make_smart`: make a SmartGraph (only available if the database is run in the Enterprise edition). Deafult is
  `False`. For smart 
  graphs, further options are available (they are ignored for non-smart graphs):
...
- _translation options_:
    - `--bulk_size`: the maximum number of vertices/edges that are internally inserted into the database in one 
database interaction, default is 10000
- _graphalytics options_ describe the input files. There are two ways to do this: by giving all three files explicitly or
  ...
  Example: the directory is `./abc/def` and the file names are `def.v` and `def.e`.
